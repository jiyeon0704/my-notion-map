...
<head>
<meta charset="utf-8" />
<title>노션 임베드용 핀 지도 (최종 안정화)</title>
...
const MIN_ZOOM = -5, MAX_ZOOM = 5; // [수정] 단순 이미지 줌 레벨
...
let initialBounds = null; // [신규] 전체 지도 영역
...
function createMarkerIcon(titleText) {
  ...
  root.innerHTML = `<div class="marker-label">${titleText || '(미선택)'}</div><div class="marker-dot"></div>`;
  ...
  Object.entries(groupedPins)
    ...
    .forEach(([type, pins]) => {
      ...
      header.innerHTML = `
          ...
          <span class="pin-group-count">${pins.length}개</span>
      `;
      ...
      pins.forEach(pin => {
          ...
          childLi.innerHTML = `
              <span class="title">└ (Y: ${pin.data.lat.toFixed(0)}, X: ${pin.data.lng.toFixed(0)})</span>
              <div class="actions">
                  <button class="delete-pin-btn" title="삭제">🗑️</button>
              </div>
          `;
          childLi.onclick = (e) => {
              if (!e.target.closest('.actions')) {
                  selectMarker(pin.id);
                  // [수정] 핀의 Y,X 좌표로 이동 (GPS 아님)
                  map.flyTo([pin.data.lat, pin.data.lng], MAX_ZOOM, { duration: 1.5 }); 
              }
          };
          const deleteBtnLi = childLi.querySelector('.delete-pin-btn');
          deleteBtnLi.onclick = async (e) => {
              e.stopPropagation();
              if (!isEditMode) { alert("편집 모드를 켜주세요."); return; }
...
function refreshTypeOptions() {
    const typeSelect = document.getElementById('typeSelect');
    typeSelect.innerHTML = '<option value="">(핀을 선택하세요)</option>';
    ...
    PIN_TYPES.forEach(type => {
        ...
        li.innerHTML = `
            ...
            <div class="type-actions">
                <button class="rename-type-btn" ${!isEditMode ? 'disabled' : ''}>✏️</button>
                <button class="delete-type-btn" ${!isEditMode ? 'disabled' : ''}>🗑️</button>
            </div>
        `;
...
/********** 3. 초기화 및 이벤트 리스너 **********/
// [수정] 플러그인 로드 대기 제거, DOMContentLoaded로 변경
document.addEventListener('DOMContentLoaded', async () => {
  try {
    const { w, h } = await getImageSize(MAP_IMAGE_URL);
    
    // [수정] CRS.Simple로 지도 초기화
    map = L.map('map', {
      crs: L.CRS.Simple, // [수정] 단순 이미지 좌표계
      minZoom: MIN_ZOOM,
      maxZoom: MAX_ZOOM,
      zoomControl: true, // 기본 줌 컨트롤
    ...
    map.on('click', async (e) => {
      ...
      // [수정] 클릭한 (Y, X) 픽셀 좌표를 바로 사용
      const docRef = await addDoc(collection(db, COLLECTION_PATH), {
        title: '', 
        lat: e.latlng.lat, // 이미지 Y좌표
        lng: e.latlng.lng, // 이미지 X좌표
        updatedAt: serverTimestamp()
      });
    ...
    document.body.innerHTML = `<div style="padding:20px; color:red">오류 발생: ${e.message}</div>`;
  }
});
...
.panel-content { padding: 15px; flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 20px; }
...
.marker-root { position: relative; width: 22px; height: 22px; }
.marker-dot {
...
.reset-view-btn {
  ...
  margin-top: 5px !important; /* 줌 컨트롤과의 간격 */
}
...
@media (max-width: 768px) {
  ...
  .panel-content { padding: 10px; gap: 15px; }
...
<span class="edit-label">편집 모드</span>
<label class="switch">
...
<div class="panel-content">
  <div class="section">
    ...
    <div class="type-input-group">
      <input id="newTypeInput" type="text" placeholder="새 종류 이름 입력" />
      <button id="addTypeBtn">추가</button>
    </div>
  ...
  <div class="section pin-list-area">
    <h3 style="display: flex; justify-content: space-between; align-items: center;">
        <span>📍 등록된 핀 목록</span>
        <span style="font-size: 13px; color: #333;">총 <span id="totalPinCount" style="font-weight: bold;">0</span>개</span>
    </h3>
    <div class="search-bar">
      <input type="search" id="pinSearchInput" placeholder="종류 이름으로 검색...">
    </div>
